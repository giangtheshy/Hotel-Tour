...
import { checkValueRegister, changeIconValidate, logOutAccount } from "../User/UserUI.js";
import { saveLocalStorage, getLocalStorage, addLocalStorage, editLocalStorage } from "../localstorage/localstorage.js";
...
const handleEventProfile = () => {
    profileBtn.addEventListener("click", () => {
        ...
        changePassword.addEventListener("click", () => {
            alert("warning", "Can't change !");
        });
    });
};
const nameProfile = document.querySelector(".name-text");
const emailProfile = document.querySelector(".email-text");
const setUpProfile = (user) => {
    nameProfile.innerHTML = user.userName.split("@")[0];
    emailProfile.innerHTML = user.userName;
};
...
registerForm.addEventListener("submit", (e) => {
    e.preventDefault();
    if (
        objAllTrue(listFlagRegister) === true &&
        agreeTermsRegister.checked === true &&
        validationUserNameRegister(userNameRegister.value) === true
    ) {
...
const objAllTrue = (obj) => {
    for (let prop in obj) if (!obj[prop]) return false;
    return true;
};
